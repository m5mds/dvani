...
<div class="mobile-drawer" id="mobileDrawer" aria-hidden="true" inert>
  <div class="mobile-panel">
    <button class="mobile-close" type="button" aria-label="إغلاق القائمة"><span></span><span></span></button>
...
<section class="section service-atelier-section">
  <div class="section-inner service-atelier">
    <div class="service-atelier-copy reveal">
      <p class="section-label">نطاق العمل</p>
      <h2>كل خدمة يجب أن تخدم يوم الافتتاح وما بعده.</h2>
      <p class="lead">نرتب القرار من أول قراءة للمساحة حتى آخر قطعة تأثيث، حتى لا يظهر التصميم كأفكار منفصلة بل كمسار واحد قابل للتسليم.</p>
    </div>
    <div class="service-ledger">
      ${services.map(([name, text], i) => `<article class="service-line reveal"><small>${String(i + 1).padStart(2, "0")}</small><h3>${name}</h3><p>${text}</p></article>`).join("")}
    </div>
  </div>
</section>
<section class="cta-band">
...
<section class="section">
  <div class="section-inner project-showcase">
    ...
    <div class="project-copy reveal">
      <p class="section-label">معرض المشاريع</p>
...
<section class="section proof-section client-wall-section">
  <div class="section-inner">
    <div class="section-head reveal">
      <p class="section-label">العملاء والشركاء</p>
      <h2>ثقة تظهر بهدوء.</h2>
    </div>
  </div>
  <div class="section-inner logo-grid client-wall">
...
<section class="section proof-section certificate-vault-section">
  <div class="section-inner certificate-vault-head reveal">
    <div>
      <p class="section-label">ملف الثقة</p>
      <h2>مستندات واضحة داخل إطار هادئ.</h2>
    </div>
    <p class="lead">نعرض الاعتمادات كوثائق قابلة للمراجعة، مع مساحة كافية لقراءة الصورة ومكان واضح لكل نوع مستند.</p>
  </div>
  <div class="section-inner certificate-grid certificate-vault">
    ${certificates.map(([src, title, text], i) => `<article class="certificate-card reveal"><small>${String(i + 1).padStart(2, "0")}</small><img src="${src}" alt="${title}"><div><h3>${title}</h3><p>${text}</p></div></article>`).join("")}
  </div>
</section>
<section class="cta-band">
  <div class="section-inner split">
...
<section class="section">
  <div class="section-inner form-layout">
    <div class="reveal">
      <p class="section-label">بداية الطلب</p>
...
<section class="section">
  <div class="section-inner form-layout">
    <div class="contact-stack reveal">
      <p class="section-label">تواصل مباشر</p>
...
.menu-button{display:none;width:46px;height:46px;border:1px solid rgba(244,238,228,.2);border-radius:50%;background:rgba(244,238,228,.03);place-items:center;gap:4px;padding:12px;cursor:pointer;transition:transform .32s var(--ease-out),border-color .32s var(--ease-out),background .32s var(--ease-out),box-shadow .32s var(--ease-out)}
.menu-button span{width:18px;height:1px;background:var(--bone);border-radius:999px;display:block;transition:background .32s var(--ease-out),transform .32s var(--ease-out)}
.menu-button:hover,.menu-button:focus-visible{transform:translateY(-1px);border-color:rgba(217,182,111,.5);background:rgba(217,182,111,.07);box-shadow:0 14px 30px rgba(0,0,0,.22)}
.menu-button:hover span,.menu-button:focus-visible span{background:var(--gold-soft)}
.mobile-drawer{position:fixed;z-index:110;inset:0;background:rgba(7,7,6,.58);backdrop-filter:blur(18px) saturate(1.08);-webkit-backdrop-filter:blur(18px) saturate(1.08);opacity:0;pointer-events:none;transition:opacity .36s var(--ease-out)}
.mobile-drawer.open{opacity:1;pointer-events:auto}
.mobile-panel{position:absolute;inset-block:0;inset-inline-end:0;width:min(88vw,370px);display:flex;flex-direction:column;padding:22px clamp(18px,5vw,28px) 26px;background:linear-gradient(180deg,rgba(17,15,12,.97),rgba(8,7,6,.99));border-inline-start:1px solid rgba(244,238,228,.14);box-shadow:-30px 0 90px rgba(0,0,0,.48),inset 1px 0 0 rgba(255,255,255,.035);overflow-y:auto;transform:translate3d(-24px,0,0) scale(.982);transition:transform .5s var(--ease-out)}
.mobile-panel::before{content:"";position:absolute;inset:0;background:linear-gradient(145deg,rgba(217,182,111,.08),transparent 34%),linear-gradient(180deg,rgba(255,255,255,.035),transparent 22%);pointer-events:none}
.mobile-panel>*{position:relative}
.mobile-drawer.open .mobile-panel{transform:translateX(0)}
.mobile-close{position:relative;align-self:flex-start;width:46px;height:46px;border:1px solid rgba(244,238,228,.18);border-radius:50%;background:rgba(244,238,228,.025);color:transparent;font-size:0;cursor:pointer;display:grid;place-items:center;transition:transform .32s var(--ease-out),border-color .32s var(--ease-out),background .32s var(--ease-out)}
.mobile-close span{position:absolute;width:18px;height:1px;background:var(--bone);border-radius:999px;transition:background .32s var(--ease-out)}
.mobile-close span:first-child{transform:rotate(45deg)}
.mobile-close span:last-child{transform:rotate(-45deg)}
.mobile-close:hover,.mobile-close:focus-visible{transform:translateY(-1px);border-color:rgba(217,182,111,.55);background:rgba(217,182,111,.08)}
.mobile-close:hover span,.mobile-close:focus-visible span{background:var(--gold-soft)}
.mobile-panel nav{display:grid;gap:6px;margin:30px 0 20px}
.mobile-panel nav a{min-height:48px;display:flex;align-items:center;justify-content:flex-start;padding:0 14px;border:1px solid transparent;border-radius:14px;color:rgba(244,238,228,.78);font-size:1rem;font-weight:650;transition:transform .3s var(--ease-out),color .3s var(--ease-out),background .3s var(--ease-out),border-color .3s var(--ease-out)}
.mobile-panel nav a:hover,.mobile-panel nav a:focus-visible{transform:translateX(-3px);color:var(--bone);background:rgba(244,238,228,.045);border-color:rgba(244,238,228,.1)}
.mobile-panel nav a[aria-current="page"]{color:var(--gold-soft);background:linear-gradient(90deg,rgba(217,182,111,.1),rgba(217,182,111,.025));border-color:rgba(217,182,111,.22)}
.mobile-panel .command{width:100%;min-height:52px;margin-top:auto}
...
.logo-band .eyebrow{margin:0}
.logo-marquee{direction:ltr;overflow:hidden;mask-image:linear-gradient(90deg,transparent,#000 5%,#000 95%,transparent)}
.logo-track{display:flex;width:max-content;gap:10px;animation:logoMove 62s linear infinite;animation-delay:-12s}
.logo-group{display:flex;gap:10px}
.logo-tile{margin:0;width:146px;height:58px;flex:0 0 auto;display:grid;place-items:center;padding:10px 16px;border:1px solid rgba(244,238,228,.065);border-radius:4px;background:linear-gradient(180deg,rgba(244,238,228,.024),rgba(244,238,228,.008));box-shadow:inset 0 1px 0 rgba(255,255,255,.018)}
.logo-tile img{max-width:116px;max-height:34px;object-fit:contain;filter:saturate(.88) brightness(1.08);opacity:.72}
...
.service-atelier-copy h2{font-size:2.35rem;margin-bottom:18px}
.service-atelier-copy .lead{margin-bottom:0}
.service-ledger{display:grid;border-top:1px solid rgba(244,238,228,.12)}
.service-line{display:grid;grid-template-columns:62px minmax(150px,.34fr) minmax(0,1fr);gap:24px;align-items:start;margin:0;padding:25px 18px;border-bottom:1px solid rgba(244,238,228,.1);border-radius:4px;transition:background .34s var(--ease-out),border-color .34s var(--ease-out),transform .34s var(--ease-out)}
.service-line small{direction:ltr;color:var(--gold-soft);font-weight:800;opacity:.76}
.service-line h3{font-size:1.26rem;margin:0}
.service-line p{margin:0}
.service-line:hover{background:rgba(244,238,228,.035);border-color:rgba(217,182,111,.22);transform:translateX(-3px)}
...
.logo-card{display:grid;place-items:center;text-align:center;min-height:150px;margin:0}
...
.client-wall-section{padding-top:88px;background:radial-gradient(circle at 18% 0,rgba(217,182,111,.055),transparent 28%),#060605}
.client-wall-section .section-head{display:block;text-align:right;max-width:640px;margin-left:auto;margin-right:0;margin-bottom:34px}
.client-wall-section .section-head h2{margin-bottom:0}
.client-wall{grid-template-columns:repeat(12,minmax(0,1fr));gap:12px}
.client-wall .logo-card{grid-column:span 3;position:relative;overflow:hidden;min-height:132px;padding:24px 20px;border-color:rgba(244,238,228,.075);background:linear-gradient(180deg,rgba(244,238,228,.028),rgba(244,238,228,.01));box-shadow:inset 0 1px 0 rgba(255,255,255,.024)}
.client-wall .logo-card::before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 50% -10%,rgba(217,182,111,.08),transparent 44%);opacity:0;transition:opacity .34s var(--ease-out)}
.client-wall .logo-card:nth-child(1),.client-wall .logo-card:nth-child(2),.client-wall .logo-card:nth-child(3){grid-column:span 4;min-height:148px}
.client-wall .logo-card img{max-width:78%;max-height:58px;margin:0;opacity:.82;filter:saturate(.86) brightness(1.08);transition:opacity .34s var(--ease-out),filter .34s var(--ease-out),transform .34s var(--ease-out)}
.client-wall .logo-card:hover::before{opacity:1}
.client-wall .logo-card:hover img{opacity:1;filter:saturate(1) brightness(1.14);transform:translateY(-1px)}
.client-wall .logo-card figcaption{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap;border:0}
...
.certificate-vault-section{background:radial-gradient(circle at 82% 0,rgba(217,182,111,.07),transparent 32%),linear-gradient(180deg,#070706,#050504);border-top:1px solid rgba(244,238,228,.08)}
.certificate-vault-head{display:grid;grid-template-columns:minmax(0,.9fr) minmax(280px,.72fr);gap:42px;align-items:end;margin-bottom:34px}
.certificate-vault-head h2{margin-bottom:0}
.certificate-vault-head .lead{margin:0;color:rgba(244,238,228,.68)}
.certificate-vault{gap:16px}
.certificate-vault .certificate-card{position:relative;overflow:hidden;grid-template-columns:168px minmax(0,1fr);min-height:246px;padding:22px;border-color:rgba(244,238,228,.09);background:linear-gradient(180deg,rgba(244,238,228,.04),rgba(244,238,228,.014));box-shadow:0 34px 92px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.024)}
.certificate-vault .certificate-card::before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 0 0,rgba(217,182,111,.09),transparent 42%);opacity:.65;pointer-events:none}
.certificate-vault .certificate-card small{position:absolute;inset-block-start:18px;inset-inline-end:20px;direction:ltr;color:var(--gold-soft);font-weight:850;opacity:.72}
.certificate-vault .certificate-card img{position:relative;z-index:1;height:206px;max-height:none;padding:6px;background:#f4f0e8;border-color:rgba(244,238,228,.28);box-shadow:0 16px 48px rgba(0,0,0,.24)}
.certificate-vault .certificate-card div{position:relative;z-index:1;padding-top:24px}
.certificate-vault .certificate-card h3{margin-bottom:8px}
.certificate-vault .certificate-card p{margin:0;color:rgba(244,238,228,.66)}
.certificate-vault .certificate-card:hover{background:linear-gradient(180deg,rgba(244,238,228,.055),rgba(244,238,228,.018));border-color:rgba(217,182,111,.26)}
